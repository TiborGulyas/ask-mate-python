...
def question_list():
    if request.args:
        ...
        sorted_question_dictionary_list = sorted(
            question_dictionary_list,
            key=lambda question: question[args['order_by']],
...
def add_question(question_id='None'):
    if question_id.isdigit() is True and request.method == 'GET':
        question_dictionary_list = data_manager.get_data('question')
        ...
        new_question = {'title': request.form.get('title'), 'message': request.form.get('message'),
                        'id': data_manager.generate_id('question'), 'submission_time': data_manager.generate_time(),
...
def add_answer(question_id):
    ...
    if request.method == 'GET':
        ...
        new_answer = {'message': request.form.get('message'), 'id': data_manager.generate_id('answer'),
                      'submission_time': data_manager.generate_time(), 'vote_number': '0', 'image': '',
